...
def privacy():
    ...
    contact = db.execute("SELECT * FROM contact_info WHERE id = 1")[0]
    return render_template("privacy.html", content=policy[0]["content"] if policy else "", contact=contact)
...
def terms():
    ...
    contact = db.execute("SELECT * FROM contact_info WHERE id = 1")[0]
    return render_template("terms.html", content=terms[0]["content"] if terms else "", contact=contact)
...
def cookiepolicy():
    ...
    contact = db.execute("SELECT * FROM contact_info WHERE id = 1")[0]
    return render_template("cookies.html", content=policy[0]["content"] if policy else "", contact=contact)
...
@login_required
def admin_portfolio():
    projects = db.execute("SELECT * FROM portfolio")
    return render_template("admin_portfolio.html", projects=projects)
...
def add_project():
    ...
    db.execute("INSERT INTO portfolio (name, image) VALUES (?, ?)", name, image)
